...
| [`install-thirdparty`](install-thirdparty) | The applications Debian doesn't ship: Tailscale, Mullvad Browser, Cryptomator, yazi, Obsidian, Claude Code, Codex. Each uses a different channel, and the rationale — including what can and cannot be verified — is documented per app. The two vendor shell installers are labelled as unverified and are fetched, hashed and offered for reading rather than piped into a shell. |
