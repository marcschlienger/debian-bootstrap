...
| [`install-thirdparty`](install-thirdparty) | The applications Debian doesn't ship: Tailscale, Mullvad Browser, Cryptomator, VeraCrypt, yazi, Obsidian, Claude Code, Codex, TeX Live. Each uses a different channel, and the rationale — including what can and cannot be verified — is documented per app. The two vendor shell installers are labelled as unverified and are fetched, hashed and offered for reading rather than piped into a shell — with the better channels that exist for both written down, and why they weren't taken. |
